...
愿意是ip route出问题了，可以联系网管给删除路由，目前建议参考下方```"具体Case指南: Crontab自动删除路由，来自动化该流程"```
...
### B. 登录相关
 ...
 再次检查自己的大小写等等，如果不是
 大概率是自己不在这个container的login.user.allow里面
 联系Container的主人进行添加
...
首先进入`/opt/cuda-x.x/samples/1_Utiliies/deviceQuery`(cuda版本以你具体用到的cuda版本一致)**以sudo形式**执行deviceQuery，观察是否PASS
如果PASS了，那大概率是可以ok了

如果报错了`ERROR_CODE=100`的话，先执行`sudo ldconfig`，再执行上面的deviceQuery

如果再出现了别的错误，可以先google一下报错指令，如果没有明显解答的话联系网管
...
之后重新修改为`* 1 * * * /sbin/route del default gw 10.0.3.1`来每小时删除一次
