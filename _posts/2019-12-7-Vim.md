...
### 2024-10-29 跳转与NerdTree

Vim稳定运转一些时间后，在一次课程（[吕老师](https://kainwen.com/2024/04/13/discrete-noiseless-channel-analytical-combinatorics%e6%96%b9%e6%b3%95/)来讲的c/unix编程）之后，下定决心把vim再update一下，总结了一下主要是缺这两个功能：
1. 读代码时候的函数跳转 -> jedi.vim
2. 写代码时候的文件跳转 -> nerdTree
安装了这两个插件（用Vundle，在vimrc执行了`:PluginInstall`，没遇到太大问题），添加了以下的配置。
...
- `p` 跳到父节点
- `r` 刷新（并不太常用）
- `U` 将指针所在节点的父节点选为根节点(向外一层)

---


Jedi的是新添加的,主要发现它会在打"."的时候自动启动索引，每次写`torch.nn`的时候都会hang好久，这个比较难以接受，所以在vimrc中给取消了。以及因为mac上自动补全的快捷键`CTRL+SPACE`和切换输入法矛盾了，所以改为了`CTRL+J`，不改为`CTRL+N`的原因是它索引比较大的库的时候比较慢，当只是补全变量名的时候还是用`N`.
...
- `K`: 在函数定义的时候打开帮助文档（torch的还是比较全的，省的开网页了）
- `[Prefix]+g`: 进入函数定义(往里面一层)，其中`[Prefix]`按钮为默认的`\` (如果需要返回的话直接用vim内建的`CTRL+O` [参考](https://stackoverflow.com/questions/38362597/jedi-vim-return-to-old-position-after-jump-with-goto-assignments-definitions))
- `[Prefix]+n` 查看当前文件下所有调用某个函数的使用

![](https://github.com/A-suozhang/MyPicBed/raw/master/img/20241029160335.png)
