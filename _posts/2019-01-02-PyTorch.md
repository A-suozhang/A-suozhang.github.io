...
date:       2020-04-21            # 时间
author:     tianchen                      # 作者
header-img: img/diffusion/dnd-2.png #这篇文章标题背景图片  
catalog: true                       # 是否归档
tags:                               #标签
     - Coding
     - 常用
...
* net的state_dict存储的时候默认是不带grad的，如果指定keep_vars=True就可以存下来了
...
* 调用的是torch.nn中的Conv2d函数(这个返回的是一个nn.functional和nn.functional.conv2d等价)
  * 输入args
    ...
    * *dilation和stride之类的东西都可以是一个int或者是一个tuple*
      * 可以用来描述横纵不一样时候的情况
      * ![](https://github.com/A-suozhang/MyPicBed/raw/master/img/20191112185933.png)
* nn.Sequential默认输入只有forward的情况:
  - 手写一个mySequential并把其他参数作为`*args`来做
...
* 当我需要复制多份同一个tensor以完成broadcast的时候
  * [知乎文章 - pytorch的expand与repeat的区别](https://zhuanlan.zhihu.com/p/58109107)
  * expand不会产生新的内存，而repeat会进行显示拷贝
  * 两者的梯度一致
...
* Advanced Indexing: Equivalent GatherN-ND:
  - [参考](https://stackoverflow.com/questions/61783826/how-to-implement-tf-gather-nd-in-pytorch-with-the-argument-batch-dims)
  - ![](https://github.com/A-suozhang/MyPicBed/raw/master/img/20221006204820.png)
  - 准备好的内容: inputs:[bs,C,W,H]   locations:[bs,K,2]idx的位置，是int，由torch.where或者是torch.nonzero得到  batch_enumerate: [bs,1]
    - location拆分为： pos_h/pos_w: [bs,K]
  - inputs[batch_enumeration,:,pos_h,pos_w] -> [bs,C,12]
...
* 发现随着训练显存逐渐累积
  * 检查你的AvgMeter里面的loss是不是加了`item()`，否则是把一个带grad的loss对象给存下来了，不会被释放始终
* 注意如果要在一个tensor中sample的话，直接用`torch.random(LOW,HIGH,SIZE)`并不是sample-with-replacement的！应该使用randperm的方式
