...
* 愚蠢的我对某个已经提交过去的commit进行了修改，然后使用了amend，但是由于修改的内容与amend的这个commit冲突了，导致无法push，我pull下来之后又乱了
1. 我们首先需要回到我所希望的commit的文件状态，因为我本地的文件因为pull的automerge现在是一片混乱 ```git reset --hard $last_commit_id```
2. 然后我们要让树回到我最后一次傻逼的commit之前的状态 ```git reset --soft $commit_before_amend``` , 一定要用soft因为这样不会影响到本地的文件
3. 然后建立一个新的commit就可以了
...
* 先git fetch origin $branch
* ```git reset --hard origin $branch```

### Git diff

* 如果git add之后使用git diff是看不到东西的，如果我还想要对比, 用 ```git reset HEAD ./XXX```去unstage，很蠢
* 使用```git diff --cached```

### Git Patch

* 使用`git diff $FILE > patch_0`
* 将patch拷贝到别的地方并且apply `git apply patch_0` - 注意相对路径要对齐
