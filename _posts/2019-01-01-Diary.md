...
### TODO:
* **TODO现在在[网页版TODO](http://www.gxtodo.com/web/)进行更新**
* ~~⭐找飞哥安排一下量化的bug~~
  * 可能还需要尝试一些新的？
* ~~有可能(时间允许的话)压一下digit的网络大小？~~
  * 反正文章中说是可以的（而且理论上我们最后的场景也还没定呢）
* ~~VisDA的exmaple可以写到本子里但是估计没时间做~~
* 明天约了郭哥讨论,早上需要把几个问题specify一下 - 还有很多东西没有搞清楚
  * 现在需要拿一个硬件设计的结构出来
    * 叙述一下计算流程
      * 找一找可以优化的点
    * 叙述一下定点方式
    * 先给出一个目前网络的参数量和中间结果来定一下选型
* 学习HLS
* 几个问题
  * 反传时候的对于N做求和,以及损失函数里的1/N
  * 郭哥文章里那个fc层维度的公式
  * ~~ BN2d是不是对所有的东西都(W*H)做了同样的操作 ~~
    * 是
  * 定点精度问题(我需要存24比特的weight,但是实际拿出来做运算的还是8比特,那weight呢)
    * 新的问题,grad需要定到8比特,但是我训练会崩溃
  * 做一下对每一层的分析
